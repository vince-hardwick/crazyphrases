...
- **Next action**: Plan the next signed-in 2-player asynchronous Game slice. Reveal and completed-game history are the next likely product gap after all Turns can be submitted, but they require an owner-approved scope before implementation. Keep invite expiry, creator cancellation UI, Share Consent, friends, nudges, public discovery, broader profile/account UI, and broader social work out of the next slice unless the owner explicitly expands scope.
