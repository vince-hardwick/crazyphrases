...
- **Creator-cancellation source status**: Branch `codex/creator-cancel-multiplayer` adds source-controlled creator cancellation for Pending Games before start and Started Games before Reveal. ADR `0016` records the authority boundary. Source migration `20260619151000_creator_multiplayer_cancellation.sql` has local migration-surface coverage but has not been applied to hosted Supabase. On 2026-06-19, PR #58 was deployed to `dev` after owner approval as runtime commit `751f0ad82d26bc56ef282e7bfe76b01c23c77f85`; visible read-only dev inspection confirmed stamped runtime assets but stopped before signed-in write/cleanup smoke because the hosted migration has not yet been approved or applied.
- **Next action**: Seek owner approval for hosted creator-cancellation migration application and signed-in write/cleanup smoke in `dev`. The legacy `public.game_turns` and `public.game_entries` precondition checks documented in `docs/runbooks/supabase-auth-and-postgres.md` were run read-only on 2026-06-19 and both tables had zero rows before hosted participant-section migration application. The owner confirmed on 2026-06-19 that the live site has no users yet, so any future legacy Turn rows should be treated as smoke-test artefacts unless the owner later identifies real user-owned legacy submissions; clean them up only through an explicitly approved hosted cleanup route where needed. Keep invite expiry, Share Consent, friends, nudges, public discovery, broader profile/account UI, and broader social work out of the creator-cancellation slice unless the owner explicitly expands scope.
