...
| Multiplayer and social graph | Pending invite expiry duration accepted; most social scope deferred | Signed-in 2-player asynchronous game; Pending invite expiry duration; One-way following; Friend relationships; Manual pokes; Android app and push notifications |
...
- **Remaining risk**: Production promotion and post-production smoke remain pending explicit owner approval or the documented workflow gate. Future notification cadence or scheduled persistence of `status = 'expired'` is separate scope.
...
- **Scope guardrails**: Keep Share Consent, friends, nudges, public discovery, broader profile/account UI, and broader social work out of the completed multiplayer batch history page unless the owner explicitly expands scope.
