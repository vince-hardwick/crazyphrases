...
- **Next action**: Seek explicit owner approval before applying the participant-section hosted migration or deploying the branch to `dev`. Before hosted migration application, run the legacy `public.game_turns` and `public.game_entries` precondition checks documented in `docs/runbooks/supabase-auth-and-postgres.md`; if meaningful submitted legacy Turn data exists, pause and write an explicit data migration. Keep invite expiry, creator cancellation UI, Share Consent, friends, nudges, public discovery, broader profile/account UI, and broader social work out of the hosted validation slice unless the owner explicitly expands scope.
