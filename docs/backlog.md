...
- **Participant-section foundation status**: On 2026-06-18, branch `codex/multiplayer-execution-redesign` added the source-controlled ADR `0015` participant-section foundation locally: participant-local section dashboard buckets, section submission, completion-gated participant-scoped Reveal, durable in-app notification rows, Supabase adapter methods, migration-surface coverage, and local browser smoke coverage. Source migration `supabase/migrations/20260618192252_participant_section_multiplayer_execution.sql` has not been applied to hosted Supabase, and no `dev`, `test`, or production deployment has been triggered for this foundation.
- **Next action**: Seek explicit owner approval before applying the participant-section hosted migration or deploying the branch to `dev`. Before hosted migration application, run the legacy `public.game_turns` and `public.game_entries` precondition checks documented in `docs/runbooks/supabase-auth-and-postgres.md`. The owner confirmed on 2026-06-19 that the live site has no users yet, so any existing legacy Turn rows are smoke-test artefacts and do not require data migration; clean them up only through an explicitly approved hosted cleanup route where needed. Keep invite expiry, creator cancellation UI, Share Consent, friends, nudges, public discovery, broader profile/account UI, and broader social work out of the hosted validation slice unless the owner explicitly expands scope.
