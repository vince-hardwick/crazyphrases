...
- **Remaining risk**: Any production uploaded-avatar write smoke remains separately approval-gated. The #63 Uploaded Avatar slice intentionally excludes crop positioning, crop metadata, image-content moderation, automated safety scanning, human review queues, report queues, public-discovery safety workflows, image transcoding, metadata stripping, derived cropped-image generation, historical-avatar garbage collection, and account-deletion media-retention rules. The dev smoke left one unreferenced generated test object as a historical Uploaded Avatar because Supabase Storage correctly blocked direct SQL deletion and no Storage API cleanup tool was available in-session; this is covered by the deliberately deferred historical-avatar garbage-collection lifecycle. #64 remains the route for circular mask cropping once the upload, storage, descriptor, validation, permission, and fallback contract is in place. Revisit Uploaded Avatar retention when completed-game history renders uploaded avatars in production or when Account Deletion implementation reaches uploaded media; revisit metadata stripping and derivative generation if uploaded images create privacy, performance, or presentation problems; revisit moderation and abuse handling before Uploaded Avatars appear in public discovery surfaces or if abuse appears in signed-in game/profile contexts.
