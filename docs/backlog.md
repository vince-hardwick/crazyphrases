...
- **Remaining risk**: Uploaded avatar images are now implemented through #63 / PR #74. Circular mask cropping remains intentionally deferred to #64, and broader account-navigation or profile-surface redesign still needs a separate owning slice.
...
- **Still deferred**: Circular mask cropping for uploaded avatar images remains tracked by #64.
- **Remaining risk**: Any production uploaded-avatar write smoke remains separately approval-gated. The #63 Uploaded Avatar slice intentionally excludes crop positioning, crop metadata, image-content moderation, automated safety scanning, human review queues, report queues, public-discovery safety workflows, image transcoding, metadata stripping, derived cropped-image generation, historical-avatar garbage collection, and account-deletion media-retention rules. The dev and test smokes left unreferenced generated test objects as historical Uploaded Avatars; this is covered by the deliberately deferred historical-avatar garbage-collection lifecycle rather than bypassing Supabase Storage deletion protections. #64 remains the route for circular mask cropping once the upload, storage, descriptor, validation, permission, and fallback contract is in place. Revisit Uploaded Avatar retention when completed-game history renders uploaded avatars in production or when Account Deletion implementation reaches uploaded media; revisit metadata stripping and derivative generation if uploaded images create privacy, performance, or presentation problems; revisit moderation and abuse handling before Uploaded Avatars appear in public discovery surfaces or if abuse appears in signed-in game/profile contexts.
