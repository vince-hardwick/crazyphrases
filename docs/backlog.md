...
### Dev static-host cleanup after accidental backend-source upload

- **Deferred**: Removing the accidentally uploaded `supabase/` directory from the `dev` cPanel document root.
- **Why deferred**: The 2026-06-12 `deploy-dev.yml` run for commit `9a78d05ebcb9236c69ceef777edd6e765fd22275` uploaded `supabase/migrations/20260612152050_create_signed_in_solo_current_games.sql` before the FTPS exclude list covered source-only backend paths. Routine exclude-list fixes do not delete already-excluded remote files, and `dangerous-clean-slate` would wipe the whole selected FTP server directory.
- **Revisit when**: The site owner can approve a targeted cPanel/FTPS cleanup or an explicitly reviewed one-time clean-slate deployment against the known `dev` document root.
- **Remaining risk**: The migration SQL does not contain secrets, but the shared `dev` host may expose backend implementation details until the stray directory is removed.
