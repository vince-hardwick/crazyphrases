...
- **Deferred post-MVP crop UX**: Issue #79 tracks replacing the MVP numerical crop controls with a visual avatar cropper. The preferred interaction uses a crop box with edge and corner crop handles, plus a crop guide overlay with a rule-of-thirds grid and centre guides. This was deferred on 2026-06-25 because the derived-image storage contract and file upload/crop/save path are accepted for MVP, while the preferred visual cropper would expand scope after promotion.
