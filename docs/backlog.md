...
- **Next action**: Select the next post-PR #57 multiplayer slice from the remaining deferred scope, rather than extending the completed promotion slice. The legacy `public.game_turns` and `public.game_entries` precondition checks documented in `docs/runbooks/supabase-auth-and-postgres.md` were run read-only on 2026-06-19 and both tables had zero rows before hosted migration application. The owner confirmed on 2026-06-19 that the live site has no users yet, so any future legacy Turn rows should be treated as smoke-test artefacts unless the owner later identifies real user-owned legacy submissions; clean them up only through an explicitly approved hosted cleanup route where needed. Keep invite expiry, creator cancellation UI, Share Consent, friends, nudges, public discovery, broader profile/account UI, and broader social work out of the PR #57 promotion slice unless the owner explicitly expands scope.
