...
- Active browser-facing DTOs, repository APIs, lookup creation paths, favourite
  participant labels, notification copy generated by current SQL helpers, and
  current dashboard/history RPC JSON should use Gamer Tag for participant
  identity. Legacy-named physical columns such as `handle` and `gamer_name` may
  remain temporarily as storage mirrors until a later schema cleanup, but they
  are not product authority and must be translated at the boundary.
